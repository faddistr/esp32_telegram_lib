...
static esp_err_t telegram_http_event_handler(esp_http_client_event_t *evt)
{
      switch(evt->event_id) {
        case HTTP_EVENT_ERROR:
            TELEGRAM_LOGI(TAG, "HTTP_EVENT_ERROR");
            break;
        case HTTP_EVENT_ON_CONNECTED:
            TELEGRAM_LOGI(TAG, "HTTP_EVENT_ON_CONNECTED");
            break;
        case HTTP_EVENT_HEADER_SENT:
            TELEGRAM_LOGI(TAG, "HTTP_EVENT_HEADER_SENT");
            break;
        case HTTP_EVENT_ON_HEADER:
            TELEGRAM_LOGI(TAG, "HTTP_EVENT_ON_HEADER");
            printf("%.*s", evt->data_len, (char*)evt->data);
            break;
        case HTTP_EVENT_ON_DATA:
            TELEGRAM_LOGI(TAG, "HTTP_EVENT_ON_DATA, len=%d", evt->data_len);
        ...
        case HTTP_EVENT_ON_FINISH:
            TELEGRAM_LOGI(TAG, "HTTP_EVENT_ON_FINISH");
            break;
        case HTTP_EVENT_DISCONNECTED:
            TELEGRAM_LOGI(TAG, "HTTP_EVENT_DISCONNECTED");
            break;
    }
    return ESP_OK;
}
...
static char *telegram_io_read_all_content(esp_http_client_handle_t client)
{
...
#if TELGRAM_DBG == 1
    TELEGRAM_LOGI(TAG, "HTTP GET Status = %d, content_length = %d",
    ...
    do
    {
        ...
        if (data_read < 0)
        {
            TELEGRAM_LOGE(TAG, "Data read error: %d %d", data_read, total_data_read);
            break;
        }
        total_data_read += data_read;
    } while(data_read != 0);
#if TELGRAM_DBG == 1
    TELEGRAM_LOGI(TAG, "%s", buffer);
#endif
    return buffer;
}
...
static esp_err_t telegram_io_set_headers(esp_http_client_handle_t client, telegram_io_header_t *headers)
{
    ...
    while (headers[i].key)
    {
        ...
        if (err != ESP_OK)
        {
            TELEGRAM_LOGE(TAG, "Send message: esp_http_client_set_header failed err %d", err);
            break;
        }
        i++;
    }
 ...
 telegram_io_header_t *headers)
{
    ...
    if (err != ESP_OK)
    {
        TELEGRAM_LOGE(TAG, "esp_http_client_set_url failed err %d", err);
        return err;
    }
    ...
    if (err != ESP_OK)
    {
        TELEGRAM_LOGE(TAG, "Send file: esp_http_client_set_method failed err %d", err);
        return err;
    }
    ...
    bool is_post, const char *post_field, void *ctx, telegram_io_send_file_cb_t cb)
{
    ...
    if (err != ESP_OK)
    {
        TELEGRAM_LOGE(TAG, "telegram_io_prepare failed err %d", err);
    ...
    if (err != ESP_OK) {
        TELEGRAM_LOGE(TAG, "esp_http_client_connect failed err %d", err);
        if (io_ctx == NULL)
        {
            esp_http_client_cleanup(client);
        }
    ...
    if (post_field)
    {
        ...
        {
            TELEGRAM_LOGW(TAG, "esp_http_client_write send_len != strlen(post_field)  %d", send_len);
        }

        if (send_len < 0)
        {
            TELEGRAM_LOGE(TAG, "Error during esp_http_client_write post_field");
    ...
    if ((err == ESP_OK) && (total_len != 0) && (cb != NULL))
    {
        ...
        while (total_len)
        {
            ...
            TELEGRAM_LOGI(TAG, "chunk_size %d max_size %d total_len %d offset %d", chunk_size, max_size, total_len, offset);
            if (chunk_size > max_size)
            {
                err = ESP_ERR_INVALID_SIZE;
                TELEGRAM_LOGE(TAG, "chunk_size > max_size");
                break;
            }
            ...
            {
                TELEGRAM_LOGW(TAG, "esp_http_client_write send_len != chunk_size %d", send_len);
            }

            if (send_len < 0)
            {
                TELEGRAM_LOGE(TAG, "Error during esp_http_client_write");
                break;
            }
...
void telegram_io_read_file(const char *file_path, void *ctx, telegram_io_get_file_cb_t cb)
{
    ...
    {
        TELEGRAM_LOGE(TAG, "Wrong params");
        return;    
    }
    ...
    if (total_len < 0)
    {
        TELEGRAM_LOGE(TAG, "esp_http_client_fetch_headers failed %d", total_len);
        esp_http_client_cleanup(client);
        cb(ctx, buffer, -1, 0);
        return;   
    }
    ...
    do
    {
        ...
        if (data_read < 0)
        {
            TELEGRAM_LOGE(TAG, "Data read error: %d", data_read);
            break;
        }
...
void telegram_io_free_platform(void *ctx)
{
  if (ctx != NULL)
  {
    esp_http_client_cleanup((esp_http_client_handle_t *)ctx);
  }
}
